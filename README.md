...
3. 完整标准离线账号随机UUID生成逻辑，已实现微软OAuth设备码登录流程，支持刷新令牌自动续期，正版玩家可通过 https://www.microsoft.com/link 完成授权
...
## 当前版本状态（v1.4.4 Beta测试版）
### 新增功能
1. 微软正版登录：实现设备码流程（OAuth → Xbox → XSTS → Minecraft 全链路），支持刷新令牌持久化存储，避免重复授权。
2. 账号管理：正版登录与离线账号统一管理，切换账号无需重新授权。
3. 此版本不提供msi安装程序
